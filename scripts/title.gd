...
const PLAYER_SYSTEM : String = "あなたはアキネーターです。ユーザーが思い浮かべているキャラクターを当ててください。テーマは「生まれ変わるとしたら？」です。\n\n【質問ルール】\n- Yes/No形式の質問を1つだけ出す\n- 必ず[Q]質問文 のフォーマットで出力する\n- 以下の順番で絞り込む：①人間か動物か架空の存在か→②現実に存在するか→③強いか弱いか→④有名なキャラクターか→⑤特定の作品・職業・特徴を確認\n- ユーザーは「はい／おそらくはい／わからない／おそらくいいえ／いいえ」で答える\n- 5〜8回で必ず決定する\n\n【決定フォーマット】（確信したら必ずこれで答える）\n[PLAYER:英語名]○○に生まれ変わるんですね！素敵な選択です。\n英語名はシンプルな英単語1つ（例：dragon、ninja、robot、cat、wizard）。"
const ENEMY_SYSTEM  : String = "あなたはアキネーターです。ユーザーが思い浮かべている「嫌いなもの・怖いもの」を当ててください。\n\n【質問ルール】\n- Yes/No形式の質問を1つだけ出す\n- 必ず[Q]質問文 のフォーマットで出力する\n- 以下の順番で絞り込む：①生き物か無生物か→②目に見えるか→③大きいか小さいか→④日常にあるか→⑤具体的な特徴を確認\n- ユーザーは「はい／おそらくはい／わからない／おそらくいいえ／いいえ」で答える\n- 5〜8回で必ず決定する\n\n【決定フォーマット】（確信したら必ずこれで答える）\n[ENEMY:英語名]白い○○と黒い○○を敵として登場させますね！\n英語名はシンプルな英単語1つ（例：spider、ghost、snake、cockroach、demon）。"
# BGは追加質問禁止・1回の入力で即決定
...
func _show_bg_ui() -> void:
	...
	hint_lbl.add_theme_color_override("font_color", Color(0.7, 0.8, 1.0, 0.75))
	hint_lbl.position = Vector2(28.0, 108.0)
...
func _start_player_akinator() -> void:
	...
	_show_splash("自分の分身を\nつくります", "頭の中でイメージしてください…", _after_player_splash)

func _after_player_splash() -> void:
...
func _start_enemy_akinator() -> void:
	...
	_show_splash("敵の分身を\nつくります", "あなたの嫌いなものを教えてください…", _after_enemy_splash)

func _after_enemy_splash() -> void:
...
func _start_bg_akinator() -> void:
	...
	_waiting = false  # 前フェーズのwaitingをリセット
	_show_bg_ui()
